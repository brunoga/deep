...
import { equal, compareOrdered } from "./equal.js";
/**
 ...
 * exist to avoid.
 */
export function evaluate(root, c, keys) {
    if (!c)
        return true;
    switch (c.o) {
        case 'and':
        ...
        case 'not': {
            const sub = c.apply?.[0];
            if (!sub)
                throw new Error('malformed not condition: missing sub-condition');
            return !evaluate(root, sub, keys);
        }
    }
    const at = resolve(root, c.p ?? '', keys);
    if (c.o === 'exists')
        return at.found && at.value !== undefined;
    if (!at.found)
        throw new Error(`condition path ${c.p} does not resolve`);
    ...
    switch (c.o) {
        ...
        case 'matches': {
            if (typeof c.v !== 'string')
                throw new Error('matches requires a string pattern');
...
export function checkType(v, typeName) {
    switch (typeName) {
        case 'string':

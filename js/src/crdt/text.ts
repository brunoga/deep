...
export function runeCount(run: TextRun): number {
  return run.n !== 0 ? run.n : runes(run.value).length;
}

function runeSlice(s: string, from: number, to: number): string {
  return runes(s).slice(from, to).join('');
}

function withValue(run: TextRun, value: string): TextRun {
  const out: TextRun = { id: run.id, value, n: runes(value).length };
  if (run.prev !== undefined) out.prev = run.prev;
  if (run.deleted) out.deleted = true;
  return out;
}

/** Splits a run at an offset, giving the tail the identifier it must have. */
export function splitRun(run: TextRun, offset: number): [TextRun, TextRun] {
...
export function length(text: TextRun[]): number {
  let n = 0;
  for (const run of text) if (!run.deleted) n += runeCount(run);
  return n;
}
 ...
 */
export function ordered(text: TextRun[]): TextRun[] {
  if (text.length <= 1) return text;
  if (inSequence(text)) return text;

  const children = new Map<string, TextRun[]>();
  for (const run of text) {
    const anchor = hlc.key(run.prev ?? hlc.ZERO);
    const group = children.get(anchor);
    if (group) group.push(run);
    else children.set(anchor, [run]);
  }
  ...
  while (stack.length > 0) {
    ...
    for (let i = n - 1; i >= 0; i--) {
      ...
      if (children.has(hlc.key(charID))) push(charID);
    }
  }
...
function inSequence(text: TextRun[]): boolean {
  // The virtual root: one character, the zero identifier, which the first run
  // anchors to.
  const stack: AnchorFrame[] = [{ base: hlc.ZERO, n: 1, cursor: 0 }];
  for (const run of text) {
    for (;;) {
      const top = stack[stack.length - 1]!;
      const prev = run.prev ?? hlc.ZERO;
      const hosts =
        prev.n === top.base.n && prev.w === top.base.w
          ? prev.l - top.base.l
          : -1;
      ...
      if (stack.length === 1) return false; // anchored somewhere the walk would not be
      stack.pop();
    }
...
function canMerge(last: TextRun, curr: TextRun): boolean {
  if (Boolean(curr.deleted) !== Boolean(last.deleted)) return false;
  if (runeCount(last) + runeCount(curr) > MAX_RUN_RUNES) return false;
  const expectedID = hlc.plus(last.id, runeCount(last));
  const expectedPrev = hlc.plus(last.id, runeCount(last) - 1);
  return (
    hlc.equal(curr.id, expectedID) &&
    curr.prev !== undefined &&
...
export function mergeAdjacent(text: TextRun[]): TextRun[] {
  if (text.length <= 1) return text;
  const result: TextRun[] = [{ ...text[0]! }];
...
export function normalize(text: TextRun[]): TextRun[] {
  return mergeAdjacent(ordered(text));
}

/** The identifier of the character at a visible position. */
function idAt(text: TextRun[], pos: number): HLC {
  if (pos < 0) return hlc.ZERO;
  let at = 0;
  for (const run of text) {
    if (run.deleted) continue;
    const n = runeCount(run);
    if (pos >= at && pos < at + n) return hlc.plus(run.id, pos - at);
    at += n;
  }
  return hlc.ZERO;
}
...
function splitAt(text: TextRun[], pos: number): TextRun[] {
...
export function insert(
  ...
  if (!hlc.isZero(prevID)) run.prev = prevID;
...
export function remove(text: TextRun[], pos: number, count: number): TextRun[] {
  ...
  for (const run of inOrder) {
    if (run.deleted) continue;
    const n = runeCount(run);
    if (at >= pos && at + n <= pos + count) run.deleted = true;
    at += n;
  }
 ...
 * differently.
 */
export function mergeRuns(a: TextRun[], b: TextRun[]): TextRun[] {
  const all = [...a, ...b];
  const splits = new Map<string, Set<number>>();
  const baseKey = (id: HLC): string => `${id.n} ${id.w}`;
  const addSplit = (id: HLC, at: number): void => {
    const k = baseKey(id);
    const set = splits.get(k);
    if (set) set.add(at);
    else splits.set(k, new Set([at]));
  };
  ...
  const combined = new Map<string, TextRun>();
  const take = (id: HLC, candidate: TextRun): void => {
    const k = hlc.key(id);
    const existing = combined.get(k);
    if (existing) {
      // The same characters seen twice: keep one, but a deletion anywhere is
      // a deletion everywhere.
      if (candidate.deleted) existing.deleted = true;
      return;
    }
  ...
  for (const run of all) {
    ...
    for (const s of relevant) {
      ...
      const id: HLC = { w: run.id.w, l: currentLogical, n: run.id.n };
      const piece = withValue(run, runeSlice(currentValue, 0, offset));
      piece.id = id;
      if (currentPrev !== undefined) piece.prev = currentPrev;
      else delete piece.prev;
      take(id, piece);
...
export function markDeleted(text: TextRun[], ranges: DeletedRange[]): TextRun[] {
  if (ranges.length === 0) return text;
  const contains = (r: DeletedRange, id: HLC): boolean => {
    if (id.n !== r.id.n || id.w !== r.id.w) return false;
  ...
  for (const run of text) {
    ...
    for (let i = 1; i <= n; i++) {
      if (i === n || deleted[i] !== deleted[start]) {
        let piece = run;
        if (start > 0) piece = splitRun(run, start)[1];
        if (i < n) piece = splitRun(piece, i - start)[0];
        piece = { ...piece };
        if (deleted[start]) piece.deleted = true;
        else delete piece.deleted;
        out.push(piece);
        start = i;
      }
    }

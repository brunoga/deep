 ...
 */
export function merge(base: Patch, other: Patch, resolver?: ConflictResolver): Patch {
  const latest = new Map<string, { op: Operation; isOther: boolean }>();

  const take = (ops: Operation[], isOther: boolean): void => {
    for (const op of ops) {
  ...
  for (const [path, entry] of [...latest]) {
    ...
    for (const [otherPath, otherEntry] of latest) {
      if (path === otherPath || otherEntry.isOther === entry.isOther) continue;

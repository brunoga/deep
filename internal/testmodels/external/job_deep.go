		...
		if (t.Owners == nil) != (other.Owners == nil) {
			...
			if other.Owners != nil {
				for k, v := range other.Owners {
					...
					switch {
					case !ok:
						p.Operations = append(p.Operations, deep.Operation{Kind: deep.OpAdd, Path: at + "/owners/" + deep.EscapePathKey(fmt.Sprintf("%v", k)), New: v})
					case (oldV == nil) != (v == nil):
						p.Operations = append(p.Operations, deep.Operation{Kind: deep.OpReplace, Path: at + "/owners/" + deep.EscapePathKey(fmt.Sprintf("%v", k)), Old: oldV, New: v})
					case oldV != nil && !oldV.Equal(v):
						sub := oldV.Diff(v)
						prefix := at + "/owners/" + deep.EscapePathKey(fmt.Sprintf("%v", k))

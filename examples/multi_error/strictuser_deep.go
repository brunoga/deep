...
func (t *StrictUser) Patch(p deep.Patch[StrictUser], logger *slog.Logger) error {
	if logger == nil {
		logger = slog.Default()
	}
	if p.Guard != nil {
...
func (t *StrictUser) applyOperation(op deep.Operation, logger *slog.Logger) (bool, error) {
	...
	case "/":
		if op.Strict && op.Old != nil && (op.Kind == deep.OpReplace || op.Kind == deep.OpRemove) {
			_match := false
			if old, ok := op.Old.(StrictUser); ok {
		...
		if op.Kind == deep.OpReplace {
			if v, ok := deep.ValueAs[StrictUser](op.New); ok {
				*t = v
				return true, nil
			}
		}
...
func (t *StrictUser) evaluateCondition(c condition.Condition) (bool, error) {
	switch c.Op {
	case "and":
...
func (t *StrictUser) Clone() *StrictUser {
	res := &StrictUser{
		Name: t.Name,
		Age:  t.Age,
	}

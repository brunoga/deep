	...
	case "/env", "/Environment":
		if op.Kind == deep.OpReplace && op.Strict && op.Old != nil {
			...
			if _oldV, ok := op.Old.(string); ok {
				_match = t.Environment == _oldV
			}
			if !_match {
				_match = deep.EqualCoerced(t.Environment, op.Old)
			}
			if !_match {
	...
	case "/features", "/Features":
		if op.Kind == deep.OpReplace && op.Strict && op.Old != nil {
			...
			if old, ok := op.Old.(map[string]bool); ok {
				_match = deep.Equal(t.Features, old)
			}
			if !_match {
				_match = deep.EqualCoerced(t.Features, op.Old)
			}
			if !_match {
...
func (t *Config) evaluateCondition(c condition.Condition) (bool, error) {
	...
	case "/env", "/Environment":
		...
		if c.Op == "type" {
			...
			return condition.CheckType(t.Environment, tn), nil
		}
		if c.Op == "matches" {
			pat, ok := c.Value.(string)
			if !ok {
			...
			return regexp.MatchString(pat, fmt.Sprintf("%v", t.Environment))
		}

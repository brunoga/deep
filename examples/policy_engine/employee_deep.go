...
func (t *Employee) applyOperation(op deep.Operation, logger *slog.Logger) (bool, error) {
	...
	case "/":
		...
		if op.Kind == deep.OpReplace {
			if v, ok := deep.ValueAs[Employee](op.New); ok {
				*t = v
				return true, nil
			}
		}
	...
	case "/role", "/Role":
		if op.Kind == deep.OpReplace && op.Strict && op.Old != nil {
			...
			if !_match {
				_match = deep.EqualCoerced(t.Role, op.Old)
			}
			if !_match {
...
func (t *Employee) evaluateCondition(c condition.Condition) (bool, error) {
	...
	case "/role", "/Role":
		...
		if c.Op == "type" {
			...
			return condition.CheckType(t.Role, tn), nil
		}
		if c.Op == "matches" {
			pat, ok := c.Value.(string)
			if !ok {
			...
			return regexp.MatchString(pat, fmt.Sprintf("%v", t.Role))
		}
		_sv, _ok := c.Value.(string)
		if !_ok {
